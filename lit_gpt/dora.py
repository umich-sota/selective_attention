...
class LoRALinear(LoRALayer):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if r > 0:
            ...
            self.dora_m = nn.Parameter(self.linear.weight.data.norm(p=2, dim=0, keepdim=True))
    ...
    def get_dora_m(self) -> torch.Tensor:
        """Return merged lora_A and lora_B matrices with the same shape as the pretrained weights."""
        return self.dora_m
    ...
    def merge(self) -> None:
        ...
        if self.r > 0 and not self.merged:
            ...
            denominator = self.linear.weight.data.norm(p=2, dim=0, keepdim=True)
            directional_component = self.linear.weight.data / denominator
            self.linear.weight.data  = dora_m * directional_component     
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        combined_weight = pretrained + lora
...
class Config(BaseConfig):
    ...
    def mlp_class(self) -> Type:
        return getattr(lit_gpt.dora, self._mlp_class)
